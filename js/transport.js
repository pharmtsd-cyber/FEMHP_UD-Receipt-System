...
async function receiveDoc(signId) {
  ...
  if (isConfirmed && formValues) {
    ...
    if (res.success) {
      ...
      setTimeout(() => { refreshPharmaDocs(); }, 1500);
    } else {
...
if(btnRefreshPending) btnRefreshPending.addEventListener('click', refreshPharmaDocs);
  ...
  if(pBarcodeInput) {
    pBarcodeInput.addEventListener('keypress', async (e) => {
      if (e.key === 'Enter') {
        ...
        payload.timeString = `${nowTime.getHours().toString().padStart(2,'0')}:${nowTime.getMinutes().toString().padStart(2,'0')}:${nowTime.getSeconds().toString().padStart(2,'0')}`;
